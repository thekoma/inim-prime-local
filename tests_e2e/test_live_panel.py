...
async def test_integration_loads_and_creates_entities(hass, panel_config):
    ...
    volts = [s for s in states if s.entity_id.startswith("sensor.") and s.attributes.get("unit_of_measurement") == "V"]
...
async def test_declutter_hides_unused_areas_and_outputs(hass, panel_config):
    ...
    factory = [e for e in panels if e.original_name and _FACTORY_AREA.fullmatch(e.original_name.strip())]
    ...
    assert outputs and all(disabled(e) for e in outputs), "output switches must be disabled-by-default"
    ...
    print(f"[DECLUTTER] hidden: {len(factory)} factory areas, {len(outputs)} output switches (+ their memory sensors/buttons)")


@pytest.mark.skipif(os.environ.get("INIM_E2E_ARM") != "1", reason="set INIM_E2E_ARM=1 to run the Box arm/disarm E2E")
async def test_box_arm_disarm_roundtrip(hass, panel_config):
    ...
    box = next(
        (s for s in hass.states.async_all()
         if s.entity_id.startswith("alarm_control_panel.") and "box" in (s.attributes.get("friendly_name", "").lower())),
        None,
    )
    ...
    await hass.services.async_call(
        ...
        {"entity_id": box.entity_id}, blocking=True,
    )
    await hass.async_block_till_done()
    # restore immediately
    await hass.services.async_call(
